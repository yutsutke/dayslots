...
- ▶ **次の回の最初に**＝①本人が v16 で1つ記録 → `select review->>'text' from koma_docs where id='default'` を読んで、振り返りに使える形か一緒に確かめる ②別の端末（iPhone）でつないで「外の写しを取り込む」を試す（v15 の初回の聞き方が実機で分かりやすいか）。
- ▶ **写真を使い始める前に必ず**＝サムネを記録の外（IndexedDB）へ出す（いまは記録の中に画像を文字で埋めている＝端末の保存箱 約500万文字が2か月ほどで溢れ、写しも太る）。
